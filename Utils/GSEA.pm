...
sub runGSEA(){
	...
	$plot_top_x,$rnd_seed,$set_max,$set_min,$zip_report,$GSEAoutDir,$inputFile,$comparisonFile,$gseaPath,$gseaChip,$gseamaxMemory,$comparison)=@_;
	
	use Env qw(PATH);
	#$PATH.=":".$peakAnnotatorPath;
	#$ENV{'PATH'}.=":".$peakAnnotatorPath;

	my $command="java -cp .:".$gseaPath." -Xmx".$gseamaxMemory." xtools.gsea.GseaPreranked ";
