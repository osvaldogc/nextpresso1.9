...
sub executeDESeqANDcomputeCorrelationAndPCA($$$$$$$$){
	...
	my $command="R --vanilla < ".$scriptName." > ".$logFile." 2>&1";
	...
	#creates EXCEL file
	createExcel($diffExp_file,$treatmentName,$alpha);	
	...
	$command="R --vanilla < ".$corrScriptFile." > ".$logFile." 2>&1";
	print "[executing] ".$command."\n";		
	system($command);
